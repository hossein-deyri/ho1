...
import { IAuth } from "@/types/model/redux/auth";
import { useAppSelector } from "@/redux/hooks";
import { AcceptGenerateToString, isIncludedType } from "@/tools/pure-function/accept-file-generate";
import { SuccessToast } from "@/components/toast/function-toast";
      ...
      if (props.isTusProtocol) {
        if (props.multiple) {
          Object.entries(e?.dataTransfer?.files!).map(([key, value]: any) =>
            setUpload(value, {
              endpoint: `${import.meta.env.VITE_FALLBACK_URL}/${END_POINTS.MEDIA_MANAGEMENT}${props.url}`,
              headers: {
                "X-VERSION": `${import.meta.env.VITE_VERSION}`,
                "X-REALM": "smartbss",
          ...
          setUpload(e?.dataTransfer?.files!.item(0)!, {
            endpoint: `${import.meta.env.VITE_BASE_URL}/${END_POINTS.MEDIA_MANAGEMENT}${props.url}`,
            headers: {
              "X-VERSION": `${import.meta.env.VITE_VERSION}`,
              "X-REALM": "smartbss",
              Authorization: "Bearer " + token.access_token,
      ...
      <>
        ...
        >
          <UploadStyled
            ...
            height={props.height}
            width={props.width}
            className={`${props.className} p-1 rounded-lg backdrop-blur box-sh`}
          >
          ...
          {succees === false &&
            Object.entries(props?.value).map(([key, value]: any) => (
              <>
                <ShowItemNameNewStyle
                  className="backdrop-blur box-sh"
                ...
                >
                  ...
                  <div className="flex flex-col">
                    <span className="showitem-name text-xs text-left">
                      {value.name}
                    </span>
            ...
            Object.entries(props?.value).map(([key, value]: any) => (
              <>
                <SuccessInfoAfterSubmit
                  ...
                  className="backdrop-blur box-sh text-center rounded-lg my-1.5 px-6 py-2.5 h-10 text-xs"
                >
                  <div className="item-inner">
                    <BsCheck2Circle color="green" size={20} />
                    <span className="text-sm">{t(messages.completed)}</span>
                  </div>
          ...
          {isIncludedTypeFormat && (
            <ShowErrorFileUnderButton
              style={{ width: `${props.width}` }}
              className="text-center rounded-lg my-1.5 px-6 py-2.5 h-10 text-xs backdrop-blur box-sh"
            >
